  ...
  return <div id={`plan-content-${model.siteId}`} className={`plan-workbench ${expanded ? 'is-expanded' : ''}`}>
    ...
    <div className="plan-workbench-grid">
      ...
      <aside className="plan-inspector" aria-label="Plan evidence and features">
        ...
        <div className="plan-feature-list"><h4>Explore the plan</h4><p>Select a feature here or in the model.</p>{model.features.some((feature) => feature.kind === 'hatched') && <p className="plan-outline-key plan-hatched-key"><i className="plan-feature-dot is-hatched" /> Sage walls follow hatched fabric in the source. Colour distinguishes the drawing convention; its date is not assigned.</p>}{model.features.some((feature) => feature.kind === 'outline') && <p className="plan-outline-key"><i className="plan-feature-dot is-outline" /> Copper lines follow the drawing; their width does not represent wall thickness.</p>}<fieldset aria-label="Documented plan features">
